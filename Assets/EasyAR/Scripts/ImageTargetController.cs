...
{
    [SerializeField]
    List<GameObject> spawnableObjects; // list of all the objects that gets spawned off an image target
    ...
    /// <summary>
    /// Loads the objects that should be displaced when the camera see's the image target as children
    /// </summary>
    /// <param name="tracker"></param>
    private void loadChildren(int tracker = 0)
    {
        if (tracker == 1)
        {
            for (int i = 0; i < transform.childCount; i++)
            {
                GameObject.Destroy(transform.GetChild(i).gameObject); // destroys children when camera loses target to keep memory healthy
            }
        }
        else
        {
            ...
            {
                if(spawnableObjects[i].name == "ThirdPersonController")
                {
                    Instantiate(changeColourManager.getCharacter(), this.transform);
                }
                else
                {
                    Instantiate(spawnableObjects[i], this.transform); // instantiates the objects through a list as children
                }
            }
        }
    }
